...
inode_ptr directory::mk_root_dir() {
   ...
   auto par = dynamic_cast<directory *>
   (dir.get()->get_contents().get());
   par->dirents["."] = dir;
   par->dirents[".."] = dir;
   par->name = "root";
...
void directory::remove(const string &filename) {
   ...
   auto fileOrDir = dynamic_cast<directory *>
   (inode.get()->get_contents().get());
   if (fileOrDir != nullptr) {
      auto diren = fileOrDir->get_dirents();
...
const inode_ptr directory::search
(wordvec pathname, inode_state &state) {
    if (pathname.size() == 0) {
   ...
   auto searchdir = dynamic_cast<directory *>
   (searchnode.get()->get_contents().get());
   try {

...
GLfloat window::border = 4;
GLfloat window::moveby = 4;
...
void object::draw_num(int num) {
    DEBUGF('g',"draw_num")
    pshape->draw_num(center, num);
}
...
void object::move (GLfloat delta_x, GLfloat delta_y) {
   center.xpos += delta_x;
    ...
    if (center.xpos < 0) {
        center.xpos = 640;
    } else if (center.xpos > 640) {
        center.xpos = 0;
    }
    if (center.ypos < 0) {
        center.ypos = 480;
    } else if (center.ypos > 480) {
        center.ypos = 0;
    }
}
...
void window::display() {
   ...
   for (auto i = 0; static_cast<size_t >(i) < objects.size(); ++i) {
        auto obj = objects[i];
        obj.draw_num(i);
    }
   window::objects[window::selected_obj].draw_outline();
   mus.draw();
   glutSwapBuffers();
}
...
void window::keyboard (GLubyte key, int x, int y) {
   ...
   switch (key) {
      ...
      case 'H': case 'h':
         ...
         objects[selected_obj].move(-moveby, 0);
         break;
      case 'J': case 'j':
         //move_selected_object (
         objects[selected_obj].move(0, moveby);
         break;
      case 'K': case 'k':
         //move_selected_object (
         objects[selected_obj].move(0, -moveby);
      ...
      case '5': case '6': case '7': case '8': case '9':
         //select_object (key - '0');
         if (static_cast<size_t>(key - '0') > objects.size()-1){
               break;
         }
         selected_obj = key - '0';
         objects[selected_obj].draw();
         objects[selected_obj].draw_outline();
         break;
      default:
